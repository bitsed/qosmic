...
template <typename T> class Lunar
{
	...
	private:
		...
		static int thunk(lua_State *L)
		{
			...
			// check if stopped
			if (obj->context()->thread()->stopping())
	...
	public:
		static void dumpstack(lua_State* L)
		{
			...
			logInfo(QString("Lunar<%1>::dumpstack : stack has %2 elements:")
					.arg(T::className).arg(top));
			for (int n = 1 ; n <= top ; n++)
				logInfo(QString("[%1] : %2").arg(n).arg(luaL_typename(L, n)));
		}
};
}
#endif
